...
public class HdfcPayUBizServiceImpl implements PGInterface {
	private static final Logger responselogger = LoggerFactory.getLogger("org.in.com.controller.pgtrace");
	// Base URL
	private static String PAY_U_MONEY_BASE_URL_SECURE = "https://test.payu.in";
	private static String PAY_U_MONEY_BASE_URL = "https://test.payu.in";
	...
	public void packPaymentRequest(AuthDTO authDTO, OrderDTO order) {
		...
		try {
			...
			responselogger.info("PPR: " + order.getTransactionCode() + Text.SINGLE_SPACE + order.getGatewayFormParam() + Text.SINGLE_SPACE + payuMoneyHashRequest);
		}
	}
	...
	public Map<String, String> verifyPaymentOrder(AuthDTO authDTO, OrderDTO orderDTO) {
		...
		try {
			...
			responseJson = (String) response.getEntity(String.class);
			if (StringUtil.isNotNull(responseJson)) {
				JSONObject jsonResponse = JSONObject.fromObject(responseJson);
			...
			responselogger.info(orderDTO.getOrderCode() + " verifyPaymentOrder: " + responseJson + " Status:" + responseMap);
			System.out.println(orderDTO.getOrderCode() + " verifyPaymentOrder: " + responseJson + " Status:" + responseMap);
	...
	public void refund(AuthDTO authDTO, RefundDTO refund) {
		...
		try {
			...
			values.add("var1", refund.getGatewayTransactionCode());
	...
	public static void main(String a[]) {
		...
		orderDTO.setOrderCode("AONK6FDG38811");
		PaymentGatewayCredentialsDTO gatewayCredentials = new PaymentGatewayCredentialsDTO();
		gatewayCredentials.setAccessCode("7rnFly");
		gatewayCredentials.setAccessKey("pjVQAWpA");
		orderDTO.setGatewayCredentials(gatewayCredentials);
		HdfcPayUBizServiceImpl bizServiceImpl = new HdfcPayUBizServiceImpl();
		bizServiceImpl.verifyPaymentOrder(authDTO, orderDTO);
	}
}
